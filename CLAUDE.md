...
- **THE FRESH-RUN LOOK IS PART OF THE TASK, not a courtesy** (entered at the
  E5.4 approval, 2026-08-23; **its tally CORRECTED at the G.2 approval,
  2026-08-25 — see below, and see correction-drift instance (r): a rule about
  looking whose own tally nobody looked at**). A stated test limit is a PLACE
  TO LOOK, and it has found something in TWO of the FOUR looks on record —
  this line read "both times anyone looked" and was already false one task
  after it was written. The two that FOUND something: E5.3's page
  ...
  looking at a fresh run, deliberately, at the end. **The two that found
  NOTHING are why the tally matters:** E5.6-7's look through the rehearsal
  URL ("Nothing new was found", in its own bold), and G.2's, which found
  nothing in the change AND produced the mechanism's first FALSE POSITIVES —
  two apparent findings (a missing footer attribution; a blank map) that were
  the session's OWN INSTRUMENTS: a downscaled screenshot that cut the footer
  off, and a JS probe run before the page's context fetch had resolved. Both
  dissolved on POSITIVE verification (`#attrib` read in the DOM; the served
  geometry's 2 parts and 1,629 vertices), not on re-reading the same
  instrument. **So the discipline is now two-sided: look every time a task
  touches the page, report "nothing" explicitly when nothing is found, and
  when the look DOES report something, confirm it positively before believing
  it — the instrument is in scope exactly as the check is.**
- **A GUARD ON THE WRAPPER IS NOT A GUARD ON THE VALUE** (G.2, 2026-08-25).
  When a task UNIFIES two implementations of one rule, "they are now the same
  function" and "the surviving function is right" are DIFFERENT CLAIMS, and a
  test can assert the first while nothing asserts the second. *Evidence, one
  instance, measured:* G.2 unified two earth radii (`viewer_model`'s 6371.0
  and the engine's IUGG 6371.0088) behind one `haversine_km`, and
  `test_one_predicate_serves_both_consumers_on_one_earth` asserted
  `viewer_model._haversine_km is haversine_km`. Mutation M4 — putting 6371.0
  back INSIDE the unified function — passed all nine tests: identity still
  held (both callers still shared one function), and 1.4 ppm sits below every
  other tolerance in that module. Re-measured at the approval with the pin
  deselected: **8 passed, 1 deselected** — the other tests genuinely cannot
  see it, and the MECHANISM is worth carrying because it is what makes the
  mutation narrow: M4 touches only `haversine_km`, while the AREA functions
  (`ring_area_km2`, `grid_predictable_area_km2`) read `EARTH_RADIUS_KM`
  DIRECTLY. So a mutation of the CONSTANT ITSELF is a broader, different
  mutation, and the area pins DO catch that one. Closed with a literal pin
  (one degree = 111.195080 km, vs 111.194927 on the old earth). **Where this DOES NOT belong, checked before
  writing it here:** not the unreachable-check sub-pattern (nothing refused
  first — the check ran and passed); not fixture degeneracy (no fixture, no
  coinciding statistics — rule 4's instances are all DATA fixtures whose
  symmetry makes claims collide); and not cleanly coverage-that-isn't, since
  the identity assertion is real and discriminating (a copied body would fail
  it) and the name's "on one earth" arguably claims UNITY, which the mutation
  preserves. **It gets a corollary and not an inventory row because it has ONE
  instance, and every row in that table was opened with at least two** —
  config-vouched-for-by-nothing waited until ×3. If a second instance
  arrives, it earns a row. *A caveat worth carrying: the fix's own first
  assertion (`one_degree == pi * EARTH_RADIUS_KM / 180`) is SELF-REFERENTIAL
  and would survive a change to the constant itself; only the literal pin
  beside it is load-bearing against that broader mutation. Two asserts, two
  different mutations — keep both.*
...
| **Trigger mismatch** — a BACKLOG trigger names an event UNRELATED to its own remedy, so the entry waits on something the work never needed and expires unnoticed. A defect in the RECORD-KEEPING mechanism, filed here rather than in the BACKLOG's conventions because *deferral without a landing spot* is already a record-keeping class in this table and this is its sibling: that one is about a disposition with no home, this one about a home whose alarm is wired to the wrong door (named at OBS.1, 2026-08-26) | **×2 demonstrated, and measured as SYSTEMIC** | (a) **LITERATURE's evidence observer** — the remedy was ONE negation fixture needing no data, no decision and no delivery, but its trigger read "before Track G supplies any cited value". The work was possible from the day it was found (P2.CLOSE, 2026-08-20); the trigger lapsed at G.3 (2026-08-24) and was missed at that approval AND during G.2 the next day. (b) **the AOI entry** — its trigger read "before Phase-2 prediction surfaces", and Phase 2 ended producing NONE, so the condition could never become true while the work (defining the AOI) never depended on surfaces existing; rewritten at the E3.0 approval. **The adjacent evidence, not counted as mismatches because those triggers DID fire:** the E2.X disposition audit found **4 EXPIRED** triggers in one pass (coverage boundary + sole-observer hygiene on "after E2.0"; two §4 items on "Phase-2 kickoff"). **THE MEASUREMENT THAT MAKES THIS A CLASS RATHER THAN TWO ANECDOTES (OBS.1's sweep, all 62 open entries): 30 of 62 triggers name something adjacent to rather than required by the work, 24 name a condition ALREADY TRUE today, and 9 already say "any time" — which is not a trigger at all but a priority wearing one.** The diagnostic question, cheap and mechanical: *what would have to become true for the WORK to be possible?* If the answer is "nothing, it is possible now", the entry has no trigger — it has a priority, and writing a trigger anyway guarantees the miss |
| **Config vouched for by nothing** — a config file is CODE THAT NOTHING EXECUTES IN CI: its correctness is asserted only by the tool that consumes it, at the moment it consumes it, so a rule that silently matches nothing is indistinguishable from a rule that works (named at the G.3 approval, 2026-08-24; count derived, not taken from the prompt — which said "I believe there is an earlier one": the third instance found is LATER, not earlier, and ci.yml's stale comment corrected at P2.CLOSE is the same file's earlier symptom, deliberately not double-counted) | **×3** | (a) **`ci.yml`** — parsed by nothing but GitHub from Phase 0 until E5.6 (`96e3d93` added `tests/test_ci_job.py`, which parses it with PyYAML); (b) **the `.gitignore` tif rules** (`data/bathymetry/*.tif`, `data/ts6/*.tif`) — a trailing comment is part of a gitignore pattern, so both had matched NOTHING since Phase 0; found at G.3 by `git status` showing the copied rasters, proven by `check-ignore` exiting 1, fixed and re-proven; (c) **`.claude/launch.json`** — committed (`58b6b8e`) carrying absolute session-scratchpad paths that exist on no other machine; no observer anywhere, and its fix lived only in a handoff until the G.3 approval gave it a BACKLOG box. **The remedy with a record of working: make the SUITE exercise the config the way its consumer does** — (a) has it; (b)'s proof was a ONE-TIME manual check-ignore, not yet suite-borne; (c) has none. The residue (suite-borne check-ignore, the launch.json fix, the untested Dockerfile build, ci.yml's action pins) is one BACKLOG §3 entry (G.3 approval) |
...
- **WRITE THE TRIGGER BY ASKING WHAT WOULD MAKE THE WORK POSSIBLE — and if the
  answer is "nothing", write a PRIORITY, not a trigger** (OBS.1, 2026-08-26;
  the trigger-mismatch class in the table above). This is a WRITING rule, and
  it is adopted because it costs nothing: it is one question answered at the
  moment an entry is authored, by the person who already knows the remedy.
  Its measured payoff: 9 of 62 open entries already say "any time", i.e. they
  discovered the answer the long way and their trigger field is doing no work.

  **THE STRONGER FORM — every approval lists EVERY entry whose trigger is now
  true, COMPUTED from the entries — IS NOT ADOPTED, AND THE COST IS WHY**
  (convention 6: do not adopt a convention whose cost you have not measured).
  OBS.1 RAN that sweep once, over all 62 entries, and it cost **33 agents,
  ~3.5 M tokens, 998 tool calls and ~40 minutes of wall clock** — because
  judging "is this trigger true?" means reading the entry, the files it cites
  and `git log`, and because 23 of 26 "doable today" claims had to be
  REFUTED before the list was safe to publish. At that price per approval it
  would cost more than the work it protects, and an approval that skipped it
  under time pressure would be indistinguishable from one that ran it clean.

  **WHAT IS ADOPTED INSTEAD, in ascending cost:**
  1. **The writing rule above** — free, and it prevents the class at source.
  2. **Say "any time" out loud.** An entry whose condition is already true
     must SAY SO in its trigger field rather than naming a future event; the
     9 that do are self-documenting and need no sweep to find.
  3. **A sweep at PHASE BOUNDARIES, not at every approval** — the measured
     ~40 minutes is affordable a few times per phase and absurd per task, and
     the E2.X disposition audit (which found 4 expired triggers in one pass)
     is the precedent for doing it as a scheduled audit rather than a
     per-commit ritual.
  **NOT adopted, and reported rather than hidden: a mechanical check.** Only
  **24 of 62** triggers name an event a script could evaluate (a checkpoint, a
  file landing, a task committing); **35 are judgement** ("before a published
  run", "before the next metadata-shaped addition") and **3 have no trigger
  line at all**. A test over the 24 would leave the majority unwatched while
  reading as coverage — the coverage-that-isn't shape applied to the BACKLOG
  itself, which is exactly the mistake this class exists to name.
...
- Phase: **5, Track E — IN PROGRESS. E5.0 (preflight, read-only) reported
  ...
  their own origin and — until G.2 — a FIXTURE rectangle, now the REAL CCZ
  management area served from Contract 2's own file; the 35 stations from the
  manifest.
  ...
  **E5.6 + E5.7 LANDED 2026-08-23: the `run-artifact` CI job (the harness twice,
  ...
  rehearsal URL; suite 676 (703 as of G.2). CP5a IS PENDING THE DEPLOYMENT, WHICH IS KARL'S:
...
- Per-task walkthroughs are in `docs/walkthroughs/` (`phase-0-and-E1.1.md`, `E1.2.md`
  ...
  **WET.2** (per-row provenance for [03] Table 8), **WET.3** (the [02]
  ...
  the only one of the four that touches production code), `AREA.1.md`, `POS.1.md`, Track G's `G3.1.md` and `G3.2.md`, and the cross-cutting `provenance.md`). **The Phase-4/5 and Track-G files were
  ...
  matches `ls docs/walkthroughs/` exactly — 35 files then, and **39** with
  `AREA.1.md`, `G3.1.md`, `POS.1.md` and `G3.2.md` (all 2026-09-01), re-verified
  each time one was added.* Read the relevant one
