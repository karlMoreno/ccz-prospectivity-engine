...
| `test_ingestion_pipeline.py::test_cover_rows_in_the_corpus_never_carry_abundance_kg_m2` | Every COVER row post-pipeline has `abundance_kg_m2 is None` | AR-D03 / Contract 7 COVER rule | A future normalizer bug converts cover% to kg/m² and it reaches the real corpus undetected. |
| `test_ingestion_pipeline.py::test_grid_rows_in_the_corpus_are_never_observed` | Every GRID row post-pipeline has `observation_or_prediction != OBSERVED` | AR-D03 / Contract 6 | TS-6's own grid gets treated as an independent training station, making the later TS-6 benchmark comparison circular without anyone flagging it. |
| `test_ingestion_pipeline.py::test_count_rows_only_carry_abundance_with_a_recorded_mean_mass` | Every COUNT row with `abundance_kg_m2` set also has `mean_nodule_mass_g` set | AR-D03 / Contract 7 COUNT rule | A count→mass conversion happens without a documented assumption, making the derived value unauditable. |
| `test_ingestion_pipeline.py::test_mass_rows_are_training_eligible` | Every MASS row post-pipeline passes `is_training_eligible()` | AR-P02 | The corpus contains MASS rows that *look* trainable but are missing `abundance_kg_m2` or `is_open`, silently shrinking the real training set. |
...
| `test_contracts_parse.py::test_normalization_yaml_forbids_cover_kg_m2` | `normalization.yaml` declares `COVER.produces_kg_m2: false` and `GRADE.produces_kg_m2: false` | Contract 7 | The policy file itself — not just the code — could drift to allow cover→mass conversion. |
